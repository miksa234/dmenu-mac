...
static NSInteger
integer_option (const char *option, const char *value)
{
    char *end;
    errno = 0;
...
static NSColor *
color_option (const char *value)
{
    if (!value || value[0] != '#'
        || (strlen (value) != 7 && strlen (value) != 9))
...
static const char *
next_argument (int argc, char **argv, int *index)
{
    if (++*index >= argc)
        usage ();
    return argv[*index];
...
static NSString *
string_argument (int argc, char **argv, int *index)
{
    NSString *value =
...
static NSFont *
menu_font (NSString *description)
{
...
- (void)drawRect:(NSRect)dirtyRect
{
    ...
    if (options.lines) {
        NSInteger end = MIN ((NSInteger)menu.matches.count,
                             menu.firstVisible + options.lines);
        for (NSInteger i = menu.firstVisible; i < end; i++) {
            NSRect row
                = NSMakeRect (0, (i - menu.firstVisible) * options.height,
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    CGFloat promptWidth
        = o.prompt.length
              ? [o.prompt sizeWithAttributes:@{ NSFontAttributeName : font }]
                        .width
                    + 20
              : 0;
    CGFloat widest = 0;
    for (NSString *item in self.items)
        widest = MAX (
            widest,
            [item sizeWithAttributes:@{ NSFontAttributeName : font }].width
                + 20);
    o.height = MAX (o.height,
                    ceil (font.ascender - font.descender + font.leading) + 4);
    if (o.lines) {
        NSInteger available = MAX (
            1,
            floor ((visible.size.height - 2 * o.border_width) / o.height) - 1);
    ...
    CGFloat width = o.position != DMENU_POSITION_CENTER
                        ...
                        : MIN (visible.size.width,
                               MAX (o.min_width,
                                    promptWidth + widest + 2 * o.border_width));
    CGFloat height
    ...
    if (o.position == DMENU_POSITION_TOP)
        frame.origin
            = NSMakePoint (visible.origin.x, NSMaxY (visible) - height);
...
- (void)refreshMatches
{
    ...
    for (NSString *item in self.items) {
        ...
        for (NSString *token in tokens) {
            if (token.length &&
                [item rangeOfString:token options:compare].location
                    == NSNotFound) {
                found = NO;
                break;
            }
...
- (void)selectRelative:(NSInteger)delta
{
    if (!self.matches.count)
        return;
    self.selection = MAX (
        0, MIN ((NSInteger)self.matches.count - 1, self.selection + delta));
...
- (void)acceptInput:(BOOL)typed keepOpen:(BOOL)keepOpen
{
    NSString *value = typed || self.selection < 0
...
- (BOOL)handleKeyEvent:(NSEvent *)event
{
    ...
    if (control
        && ([key isEqualToString:@"j"] || [key isEqualToString:@"k"] ||
...
main (int argc, char **argv)
{
    @autoreleasepool {
        ...
        MenuController *controller =
            [[MenuController alloc] initWithOptions:options items:items];
